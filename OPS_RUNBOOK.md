...
| 6b | Portal live-resize pass gap (team ticket): enlarging a weather widget in the editor rescales its text but not its icon (`motion` rect stays 116×116) nor its box/flow (the "H:82 L:80" line lands ~150 px below the rounded box); a fresh portal boot renders it right. Measured on ATV862575515 v57→v58, 2026-09-16 | portal | the service and both clients render what the page holds; only a full reload fixes the page today |
